class ModificarSorteoForm extends HTMLElement {
                ...
                <div class="card z-index-0 fadeIn3 fadeInBottom mb-4">
                    ...
                    <form id="formulario" role="form" class="text-center" onsubmit="return false;">
                        ...
                        <div class="input-group input-group-outline mb-3 w-50 is-focused">
                            ...
                            <input class="form-control" type="number" id="tiempoApartado" name="tiempoApartado" value="1" min="1" max="5" pattern="[0-9]" required="required" onkeydown="javascript: return event.keyCode == 69 ? false : true" />
                        </div>
                        <div class="input-group input-group-outline mb-3 w-50 is-focused">
                            <label class="form-label" for="tiempoNotificaciones">Tiempo de notificaciones (días):</label>
                            <input class="form-control" type="number" id="tiempoNotificaciones" name="tiempoNotificaciones" value="1" min="1" max="2" pattern="[0-9]" required="required" onkeydown="javascript: return event.keyCode == 69 ? false : true"/>
                        </div>
    ...
    #llenarForm() {
        document.addEventListener('DOMContentLoaded', (e) => {
            ...
            fetch(
                `http://localhost:3000/sorteo/eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJub21icmUiOiJQYWNvIiwiY29ycmVvIjoiMTIzNEBob3RtYWkuY29tIiwiZGlyZWNjaW9uIjoiYXNkIiwidGVsZWZvbm8iOiIxMjI0MTEzIiwiY2l1ZGFkIjoiTmFybmlhIiwiZXN0YWRvIjoiZGUgbWV4aWNvIiwic29ydGVvcyI6W119.SiUEOo9A-9FyBoOC-Pdc4I3pTUjwM3sjmYddyfieEHg/${id}`
            )
                ...
                .then((json) => {
                    ...
                    let fechaInicioVenta = sorteo['fechaInicioVenta'].substring(
                        0,
                        10
                    );
                    ...
                    this.shadowRoot.querySelector('#precioNumeros').value =
                        precioNumeros;
                    this.shadowRoot.querySelector('#tiempoApartado').value =
                        diasLimiteApartado;
                    this.shadowRoot.querySelector(
                        '#tiempoNotificaciones'
                    ).value = tiempoRecordatorio;
                    this.shadowRoot.querySelector('#inicioVenta').value =
                        fechaInicioVenta;
                    this.shadowRoot.querySelector('#fechaSorteo').value =
                        fechaSorteo;
                    this.shadowRoot.querySelector('#finalVenta').value =
                        fechaFinal;
    ...
    #modificarSorteo() {
        ...
        let diasLimiteApartado =
            this.shadowRoot.querySelector('#tiempoApartado');
        let tiempoRecordatorio = this.shadowRoot.querySelector(
            '#tiempoNotificaciones'
        );
        ...
        const validateNumbers = (
            ...
            if (numMin > numMax) {
                alert(
                    'El número mínimo de boletos debe ser menor que el máximo'
                );
                return false;
            }
        ...
        const validateFechas = (
            ...
            if (
                fechaCreacion > fechaSorteo ||
                fechaCreacion > fechaInicioVenta ||
                fechaCreacion > fechaFinVenta
            ) {
            ...
            if (fechaInicioVenta > fechaFinVenta) {
                alert(
                    'La fecha de inicio de la venta debe ser anterior a la fecha final de la venta'
                );
                return false;
            }
            if (fechaSorteo < fechaFinVenta) {
                alert(
                    'La fecha de sorteo debe ser despues a la fecha final de la venta'
                );
                return false;
            }
            return true;
        ...
        btn.addEventListener('click', function () {
            const validarNumeros = validateNumbers(
                parseInt(numMin.value),
                parseInt(numMax.value),
                parseInt(diasLimiteApartado.value),
                tiempoRecordatorio.value
            );
            const validacionFechas = validateFechas(
                new Date(fechaInicioVenta.value),
                new Date(fechaFinVenta.value),
                new Date(fechaSorteo.value),
            ...
            if (
                validarNumeros &&
                validacionFechas &&
                validateEmpty(titulo.value, descripcion.value)
            ) {
                let img = '';
                if (localStorage.getItem('imagen')) {
                ...
                fetch(
                    ...
                    .then(function (data) {
                        ...
                        window.location.href =
                            'http://localhost:4000/views/consultarSorteo.html';
                    })
